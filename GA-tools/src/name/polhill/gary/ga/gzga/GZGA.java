...
import name.polhill.gary.ga.RankLotteryBreeder;

/**
 * GZGA
 * 
 * A GA trying to find a bitstring that is difficult to compress. Since any
 * random bitstring is difficult to compress, we pass in a population of
 * all-0 bitstings to the GA initially. This GA therefore 'finds' random
 * bitstrings...
 * 
 * @author Gary Polhill
 * 
...
public class GZGA {
   ...
   * </ol>
   * 
   * @param args Arguments to the program--see above.
   */
  public static void main(String[] args) {
    if(args.length != 5) {
      System.err
          .println("Arguments: <nSteps> <popSize> <pCrossover> <pMutate> <size>");
    }
    GZGA gzga =
      new GZGA(Integer.parseInt(args[0]), Integer.parseInt(args[1]), Double
  ...
  public GZGA(int nSteps, int popSize, double pCrossover, double pMutate,
      int size) {
    this.nSteps = nSteps;
    breeder = new RankLotteryBreeder(pCrossover, pMutate);
    gzipfunc = new GZipCostFunction();
    Gene[] initialPopulation = new Gene[popSize];
    for(int i = 0; i < initialPopulation.length; i++) {
      initialPopulation[i] = new Gene(size * 8);
    }
    ga = new GA(breeder, gzipfunc, initialPopulation);
  }

  public void run() {
    for(int i = 1; i < nSteps; i++) {
      double best_cost = ga.step(true);
      System.out.println("Generation " + i + ": Maximum length = " + best_cost
        + ", mean " + ga.getMeanCost() + ", variance " + ga.getCostVariance()
        + ", diversity " + ga.nDifferent());
    }
  }
}
